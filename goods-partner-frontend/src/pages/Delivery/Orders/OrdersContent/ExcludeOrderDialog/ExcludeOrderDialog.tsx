...
const ExcludeOrderDialog = () => {
    ...
    const handleClose = useCallback(() => dispatch(setExcludeDialogOpen(false)),
        [dispatch, setExcludeDialogOpen]);

    const handleExcludeOrder = useCallback((excludeReason:string, orderId:number) => {
    ...
    return (<Dialog sx={{minWidth: '400px'}} maxWidth={'680px'}
        ...
        <DialogActions>
            ...
            <Button onClick={()=>handleExcludeOrder(excludeReason, order.id)}
                    disabled={excludeReason.length === 0}>Зберегти</Button>
        </DialogActions>
    </Dialog>);
}
