...
const ChooseAddressDialog = () => {
    ...
    const handleClose = useCallback(() => dispatch(setAddressDialogOpen(false)),
    ...
    const isValidAddress = orderToUpdate.mapPoint.status !== MapPointStatus.UNKNOWN;
    ...
    const setCoordinates = (formattedAddress: string, lat: number, lng: number) => {
        const newMapPoint = {
            latitude: lat,
            longitude: lng,
        ...
        <DialogContent>
            ...
            >
                <TextField sx={{marginBottom: 2}} autoFocus margin="dense" label="Адреса" type="text"
                           fullWidth variant="outlined" value={orderToUpdate.address} onChange={handleAddressChange}/>

            </Autocomplete>

            <GoogleMap center={center} zoom={12} onLoad={onLoad}
