...
const RoutePointOrdersDialog = ({open, closeDialog, routePoint}: RoutePointOrdersDialogProps) => {
    const {
        data: orders,
        isLoading,
    } = useGetRoutePointOrdersQuery(routePoint.id);

    if (!orders || isLoading) {
        return <Loading/>
    }

    const commentText = orders.filter(order => order.comment).map(order => order.comment).join(",");
